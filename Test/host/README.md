...
本目录直接编译项目中的姿态、里程、钢球平衡Algorithm和正式任务状态机实现，不复制算法代码。通用状态码来自`Common/project_status.h`，`stubs`只提供主机临界区和硬件/APP边界替身，因此测试不依赖STM32芯片头文件、Keil工程或实物硬件。
...
10. 正式任务MODE3：验证+50mm越过窗口后仍能按新视觉帧确认折返、同一帧不重复计数、确认失败清零、+50不等待HOLD、-50必须HOLD后完成，并抽查MODE2行为不变。
11. 钢球参考：验证速度、加速度、加加速度限制以及到点无超调。
12. 钢球估计与平衡表：验证模型方向、创新拒绝和舵机物理行程检查。

测试使用`/W4 /WX`编译，任何主机编译警告或断言失败都会返回非零退出码。修改姿态、里程、钢球Algorithm接口或正式任务状态机后，必须重新运行本测试。
