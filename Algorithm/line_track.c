...
static int16_t LineTrack_GetAdaptiveSpeed(int16_t error)
{
    ...
    if (abs_err >= 2200) {
        return 300;   /* 大偏差/急弯 */
    } else if (abs_err >= 1500) {
        return 450;   /* 明显弯道 */
    } else if (abs_err >= 800) {
        return 650;   /* 轻中等偏差 */
    } else {
        return s_cfg.base_speed_cps;   /* 直线用 1000 */
    }
}

/* 交叉区沿用上一次偏差方向，避免在十字/8字交叉处乱选线 */
...
out->turn_cps = (int16_t)(-s_cfg.turn_max_cps * 3 / 4);
     ...
     * 对这个 HJduino 8字交叉赛道，先不要直接强制左/右选路。
...
/* 每 10ms 限制变化量，让输出更丝滑 */
out->linear_cps = LineTrack_RampI16(s_last_linear_cmd, target_linear, 60);
out->turn_cps   = LineTrack_RampI16(s_last_turn_cmd,   target_turn,   80);
